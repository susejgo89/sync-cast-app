...
export function createMediaCard(media, options = {}) {
    ...
    card.innerHTML = `
        ...
        ${!isDraggable ? `<div class="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity">
    ...
    if (!isDraggable && onDelete) {
        const deleteBtn = card.querySelector('.delete-media-btn');
        deleteBtn.addEventListener('click', () => {
            onDelete({ docId: media.id, storagePath: media.storagePath });
        });
    }
